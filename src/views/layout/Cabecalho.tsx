...
export function Cabecalho() {
  ...
  return <header className="sticky top-0 z-50 border-b border-border bg-surface/95 backdrop-blur">
    <div className="mx-auto flex h-[72px] max-w-7xl items-center gap-8 px-5 sm:px-8">
      ...
      <div className="ml-auto flex items-center gap-4 text-muted md:ml-0"><IconeSino className="h-5 w-5" /><IconeMensagem className="h-5 w-5" /><Link href="/perfil" aria-label="Meu perfil" className="flex items-center gap-1 hover:text-primary-700"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-[#f6c37a] to-primary-500 text-xs font-bold text-white">JP</span><IconeSeta className="h-4 w-4" /></Link></div>
    </div>
  </header>;
}
