alter table public.anuncios
  ...
  add column if not exists valor_unitario_centavos integer,
...
alter table public.anuncios drop constraint if exists anuncios_valor_unitario_centavos_check;
alter table public.anuncios add constraint anuncios_valor_unitario_centavos_check
  check (valor_unitario_centavos is null or valor_unitario_centavos between 1 and 2147483647);
...
alter table public.anuncios add constraint anuncios_duracao_check
  check (
    ...
    or (duracao_quantidade between 1 and 9999 and duracao_unidade in ('minutos', 'horas', 'dias', 'semanas'))
  );
...
begin
  ...
  if p_valor_unitario_centavos is null or p_valor_unitario_centavos not between 1 and 2147483647 then raise exception 'Valor unitário inválido'; end if;
  ...
  insert into public.anuncios (id, usuario_id, tipo, titulo, categoria, condicao, descricao, valor_unitario_centavos, duracao_quantidade, duracao_unidade, imagem_url)
  values (p_id, v_usuario_id, 'emprestimo', trim(p_titulo), p_categoria, p_condicao, trim(p_descricao), p_valor_unitario_centavos, p_duracao_quantidade, p_duracao_unidade, p_imagens[1]);
